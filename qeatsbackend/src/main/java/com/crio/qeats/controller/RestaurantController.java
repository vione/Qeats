...
public class RestaurantController {
  ...
  public ResponseEntity<GetRestaurantsResponse> getRestaurants(
    ...
    if (getRestaurantsRequest.getLatitude() != null 
      ...
      .findAllRestaurantsCloseBy(getRestaurantsRequest, LocalTime.now());
        ...
        getRestaurantsResponse =  restaurantService.findRestaurantsBySearchQuery(
          getRestaurantsRequest, LocalTime.now());
      }
      log.info("getRestaurants returned {}", getRestaurantsResponse);
      List<Restaurant> restaurants = new ArrayList<>();
      try { 
        restaurants.addAll(getRestaurantsResponse.getRestaurants());
      } catch (NullPointerException e) {
        return ResponseEntity.ok().body(null);
      }
      for (Restaurant restaurant: getRestaurantsResponse.getRestaurants()) {
        if (restaurant.getName().contains("é")) {
  ...
  //git pull ME_QEATS_V2_MODULE_DEBUG_V2_STUB master --allow-unrelated-histories --no-edit
